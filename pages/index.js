...
export async function getServerSideProps() {
  try {
    ...
    return {
      props: {
        featuredRooms,
        error: null // No error, so we set it to null
      }
    };
  } catch (err) {
    ...
    return {
      props: {
        featuredRooms: null,
        error: {
          message: err?.response?.data?.message || 'Failed to fetch data from the API.', // Handle error message properly
          status: err?.response?.status || 500 // Optional: Include HTTP status code
        }
      }
    };
  }
